            ...
            for (int i = 0; i + l < n; ++i) {
                ...
                if (l == 0) { //如果单个字符，必然是回文
                    ...
                    dp[i][j] = (s[i] == s[j]);
                           ...
                           // 这里难理解的一个地方是，dp[i][j]由dp[i+1][j-1]这个更短的子串决定，所以我们的状态转移方程必然是从短串到长串。
                           // 再结合子串枚举过程中，我们从每个中心点都是从短的转移到长的，并且当前中心点的长的还取决于其他中心点的短的
                           // 所以子串枚举 外面一层for是长度，里面一层for是起始点，这样就可以确保长度短的总是先于长度长的被更新。
